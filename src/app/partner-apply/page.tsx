        ...
        {!submitted ? (
          <>
            <div className="text-center mb-10">
              ...
              <h1 className="text-3xl font-black" style={{ color: "#3D3B6E" }}>
                انضم كشريك في لمتنا
              </h1>
            ...
            <div className="grid grid-cols-3 gap-4 mb-10">
              {[
                ...
                <div key={b.title} className="bg-[#F8F5FF] rounded-2xl p-4 text-center">
                  <div className="text-2xl mb-2">{b.icon}</div>
                  <div className="font-bold text-xs" style={{ color: "#3D3B6E" }}>{b.title}</div>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={form.businessName} onChange={(e) => update("businessName", e.target.value)}
                    placeholder="استوديو نور" className="w-full border-2 border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#3D3B6E]" />
                </div>
                <div>
                  <label className="text-sm font-semibold text-gray-700 block mb-2">اسم المسؤول *</label>
                  <input value={form.contactName} onChange={(e) => update("contactName", e.target.value)}
                    placeholder="أحمد النور" className="w-full border-2 border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#3D3B6E]" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  ...
                  <input type="email" dir="ltr" value={form.email} onChange={(e) => update("email", e.target.value)}
                    placeholder="info@example.com" className="w-full border-2 border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#3D3B6E] text-center" />
                </div>
              </div>
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {CATEGORIES.map((c) => (
                    <button key={c} onClick={() => update("category", c)}
                      className={`py-2 px-3 rounded-xl text-xs font-bold border-2 transition-all ${form.category === c ? "border-[#F05A7E] bg-[#FFF0F4] text-[#F05A7E]" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}>
                      {CATEGORY_LABELS[c]}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <input value={form.city} onChange={(e) => update("city", e.target.value)}
                  placeholder="الرياض" className="w-full border-2 border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#3D3B6E]" />
              </div>
              <div>
                <label className="text-sm font-semibold text-gray-700 block mb-2">ملاحظات إضافية</label>
                <textarea value={form.notes} onChange={(e) => update("notes", e.target.value)}
                  rows={3} placeholder="صِف خدماتك..." className="w-full border-2 border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#3D3B6E] resize-none" />
              </div>
          ...
          <div className="text-center py-20">
            ...
            <h2 className="text-2xl font-black mb-3" style={{ color: "#3D3B6E" }}>طلبك وصلنا!</h2>
