        ...
        {!submitted ? (
          <>
            <div className="text-center mb-10">
              ...
              <h1 className="text-3xl font-black" style={{ color: "#3D3B6E" }}>حلول الفعاليات المؤسسية</h1>
            ...
            <div className="grid grid-cols-3 gap-4 mb-10">
              {[
                ...
                <div key={b.title} className="bg-[#F8F5FF] rounded-2xl p-5 text-center">
                  <div className="text-3xl mb-2">{b.icon}</div>
                  <div className="font-bold text-sm" style={{ color: "#3D3B6E" }}>{b.title}</div>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={form.company} onChange={(e) => update("company", e.target.value)}
                    placeholder="شركة الأفق" className="w-full border-2 border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#3D3B6E]" />
                </div>
                <div>
                  ...
                  <input value={form.contact} onChange={(e) => update("contact", e.target.value)}
                    placeholder="فهد السليم" className="w-full border-2 border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#3D3B6E]" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="tel" dir="ltr" value={form.phone} onChange={(e) => update("phone", e.target.value)}
                    placeholder="+966..." className="w-full border-2 border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#3D3B6E] text-center font-mono" />
                </div>
                <div>
                  ...
                  <input type="email" dir="ltr" value={form.email} onChange={(e) => update("email", e.target.value)}
                    placeholder="hr@company.com" className="w-full border-2 border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#3D3B6E] text-center" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={form.eventType} onChange={(e) => update("eventType", e.target.value)}
                    className="w-full border-2 border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#3D3B6E] bg-white">
                ...
                <div>
                  ...
                  <input type="number" value={form.employees} onChange={(e) => update("employees", e.target.value)}
                    placeholder="٢٠٠" className="w-full border-2 border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#3D3B6E]" />
                </div>
              </div>
              <div>
                ...
                <input type="number" value={form.budget} onChange={(e) => update("budget", e.target.value)}
                  placeholder="٥٠٬٠٠٠" className="w-full border-2 border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#3D3B6E]" />
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {["مرة واحدة", "ربع سنوي", "نصف سنوي", "سنوي"].map((c) => (
                    <button key={c} onClick={() => update("cadence", c)}
                      className={`px-4 py-2 rounded-full text-sm font-semibold border-2 transition-all ${form.cadence === c ? "border-[#3D3B6E] bg-[#F8F5FF] text-[#3D3B6E]" : "border-gray-200 text-gray-500"}`}>
                      {c}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={(e) => update("notes", e.target.value)}
                  rows={3} placeholder="أي تفاصيل إضافية..." className="w-full border-2 border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#3D3B6E] resize-none" />
              </div>
          ...
          <div className="text-center py-20">
            ...
            <h2 className="text-2xl font-black mb-3" style={{ color: "#3D3B6E" }}>تم استلام طلبكم!</h2>
