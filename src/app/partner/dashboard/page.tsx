...
const STATUS: Record<string, { label: string; color: string }> = {
  PENDING: { label: "بانتظار القبول", color: "#C46878" },
  CONFIRMED: { label: "مؤكد", color: "#3D3A5C" },
  LOCKED: { label: "ضمان مقفل 🔒", color: "#3D3A5C" },
    ...
    <>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { icon: "📦", label: "طلبات جارية", value: "٣", color: "#C46878" },
            { icon: "💰", label: "أرباح هذا الشهر", value: "١٠٬٨٠٠ ر.س", color: "#3D3A5C" },
            { icon: "🔒", label: "في الضمان", value: "٦٬٥٠٠ ر.س", color: "#3D3A5C" },
            { icon: "⭐", label: "تقييمي", value: "٤.٨", color: "#C46878" },
          ].map((k) => (
            ...
            {MOCK_ORDERS.map((order) => (
              ...
              >
                <div className="flex-1">
                  <div className="font-bold" style={{ color: "#3D3A5C" }}>{order.event}</div>
                  <div className="text-xs text-gray-400">{order.client} · {order.service} · {order.date}</div>
                </div>
                <div className="text-right">
                  <div className="font-black text-base" style={{ color: "#3D3A5C" }}>
        ...
        {/* Escrow Status */}
        <section className="bg-[#3D3A5C] rounded-2xl p-6 text-white">
